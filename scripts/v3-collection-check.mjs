...
import { evaluateAuthenticity } from './v3-authenticity-lib.mjs';
...
const probableExample = JSON.parse(await fs.readFile(path.resolve('data/v3/schemas/probable-search.example.json'), 'utf8'));
const registry = JSON.parse(await fs.readFile(path.resolve('data/v3/config/source-registry.json'), 'utf8'));
const policy = JSON.parse(await fs.readFile(path.resolve('data/v3/config/authenticity-policy.json'), 'utf8'));
...
if (!probableExample.exampleOnly) throw new Error('高概率示例快照必须标记 exampleOnly=true');
const probableSource = registry.sources.find(source => source.id === probableExample.sourcePlatform);
const probableDecision = evaluateAuthenticity(probableExample, policy, probableSource);
if (probableDecision.status !== 'probable') throw new Error(`高概率示例未通过分层: ${probableDecision.status}`);
...
console.log(`Collection control valid: ${queue.length} queries; Boss access ${status.bossAccessAllowed ? 'allowed' : 'paused'}; detail and probable examples valid.`);
