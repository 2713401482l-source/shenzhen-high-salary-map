...
function RealRoleView({role}: {role: RealRoleFamily}) {
  return <article className="signal-detail">
    <div className="signal-detail-head"><div><span className="evidence-flag">真实样本</span><h2>{role.name}</h2><p>这是对当前 71 条岗位快照的归类，不代表完整市场热度，也不是历史趋势。</p></div><div className="salary-range"><small>Boss 原始范围</small><strong>{role.salaryMin}-{role.salaryMax}K</strong><span>/月</span></div></div>
    <div className="signal-facts"><div><small>岗位样本</small><strong>{role.count} 条</strong><span>{role.companies} 家企业</span></div><div><small>详情核验</small><strong>{role.verified} 条</strong><span>其余为列表页观察</span></div><div><small>薪资中位</small><strong>{role.salaryMedianMid}K</strong><span>区间中点的中位数</span></div></div>
  ...
  return <main id="main" className="inner-main">
    ...
    <div className="content-shell"><DataNotice mode={mode} />
      <section className="signal-browser">
        <div className="signal-tabs" role="tablist" aria-label="岗位方向">
          {mode === 'demo' ? demoData.roleSignals.map(role => <button key={role.id} className={demoId === role.id ? 'active' : ''} onClick={() => setDemoId(role.id)} role="tab" aria-selected={demoId === role.id}><span>{role.name}</span><small>{role.lane}</small></button>) : realRoleFamilies.map(role => <button key={role.name} className={realName === role.name ? 'active' : ''} onClick={() => setRealName(role.name)} role="tab" aria-selected={realName === role.name}><span>{role.name}</span><small>{role.count} 条样本</small></button>)}
        </div>
        {mode === 'demo' ? <DemoRoleView role={selectedDemo} /> : selectedReal && <RealRoleView role={selectedReal} />}
      </section>
      <section className="plain-callout"><div><h2>{mode === 'demo' ? `${entries.length} 个方向只用于测试浏览逻辑` : `${entries.length} 个真实岗位族群，暂时不能称为 30 个新职业`}</h2><p>{mode === 'demo' ? '等真实采集恢复后，同样的模块会换成岗位数、公司数、薪资分布和原始链接。' : '当前数据覆盖不足。少于 3 条岗位或少于 2 家公司的方向不会被包装成成熟趋势。'}</p></div><ArrowButton href="skills.html" tone="orange">继续拆能力</ArrowButton></section>
    </div>
  </main>;
}
  ...
  return <main id="main" className="inner-main">
    ...
    <div className="content-shell"><DataNotice mode={mode} />
      {mode === 'demo' ? <>
        ...
        <section className="skill-story">
          <div className="skill-story-main"><span className="evidence-flag">仅统计 7 条详情核验</span><h2>当前可核验需求里的高频能力</h2><p>列表页摘要不进入正式能力占比，避免把不完整文案当成岗位要求。</p><div className="skill-frequency">{verifiedSkillStats.slice(0, 8).map(skill => <div key={skill.name}><div><strong>{skill.name}</strong><span>{skill.count}/{verifiedJobs.length} 条</span></div><i style={{'--skill-width': `${(skill.count / topRealCount) * 100}%`} as React.CSSProperties} /></div>)}</div></div>
          <aside className="skill-combo-card"><small>当前最常见的共现</small><h3>{verifiedSkillPairs[0]?.pair.join(' + ') ?? '样本不足'}</h3><p>{verifiedSkillPairs[0] ? `在 ${verifiedSkillPairs[0].count} 条核验需求中共同出现。样本很小，只作为线索。` : '还没有足够的完整岗位要求形成共现判断。'}</p></aside>
        </section>
...
export function BenchmarkPage() {
  ...
  return <main id="main" className="inner-main">
    ...
    <div className="content-shell"><DataNotice mode={mode} />
      ...
      </> : <section className="insufficient-state"><Database /><h2>当前还没有任何单岗达到 50 条可核验需求。</h2><p>真实详情只有 {realEvidence.verified} 条，无法诚实计算“AI 产品经理”或“海外运营”的共同要求。这里保留空状态，而不是把不同岗位拼成一个假对标。</p><div><a href="jobs.html">查看现有真实岗位</a><button type="button" onClick={() => setMode('demo')}>查看结构演示</button></div></section>}
...
function OpportunityChart({points, isDemo}: {points: OpportunityPoint[]; isDemo: boolean}) {
  ...
  return <div ref={ref} className="opportunity-chart" role="img" aria-label="横轴为市场需求，纵轴为薪资上限，气泡大小代表岗位样本数量，颜色表示能力门槛" />;
}
...
export function GrowthPage() {
  ...
  return <main id="main" className="inner-main">
    ...
    <div className="content-shell"><DataNotice mode={mode} />
      <section className="map-section"><div className="map-head"><div><h2>机会地图</h2><p>{mode === 'demo' ? '完整交互使用结构假设，便于判断气泡图是否值得保留。' : '基于当前岗位数量和薪资的截面比较，不表达历史涨跌。'}</p></div><span>{mode === 'demo' ? '结构假设' : '真实样本'}</span></div><OpportunityChart points={mode === 'demo' ? demoPoints : realPoints} isDemo={mode === 'demo'} /><p className="chart-summary">横轴越右，样本需求越集中；纵轴越高，原始薪资上限越高；气泡越大，岗位样本越多。</p></section>
      {mode === 'demo' ? <section className="growth-path-section"><div className="path-selector" role="tablist">{demoData.growthPaths.map((path, index) => <button key={path.role} className={index === pathIndex ? 'active' : ''} onClick={() => setPathIndex(index)}>{path.role}</button>)}</div><div className="growth-path-head"><h2>{demoPath.role}的能力升档</h2><DemoFlag /></div><div className="growth-lane">{demoPath.stages.map(stage => <article key={stage.band}><span>{stage.band}</span><h3>{stage.title}</h3><div>{stage.skills.map(skill => <small key={skill}>{skill}</small>)}</div></article>)}</div></section> : <section className="real-paths"><h2>真实数据能支持到哪一步？</h2><p>当前只有少数岗位族群同时覆盖两个薪资档，100K 样本又高度集中于 AI 与算法。以下只展示样本分布，不把它解释成必然晋升路线。</p><div>{realRoleFamilies.slice(0, 6).map(role => <article key={role.name}><h3>{role.name}</h3><p>{role.count} 条岗位，薪资 {role.salaryMin}-{role.salaryMax}K</p><div className="band-dots">{(['30K', '50K', '100K'] as const).map(band => <span key={band} className={role.jobs.some(job => salaryBandFromMid(job) === band) ? 'has-data' : ''}>{band}</span>)}</div></article>)}</div></section>}
